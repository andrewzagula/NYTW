...
<!-- perseus:rules -->
<!-- perseus:rules v6 — managed by `perseus rules`; edit the prompt in perseus, not here -->
## Code search: query `perseus` before grep
...
structure, not just string match. Reach for it before grep whenever you need
to find *where* something lives or *how* a behaviour is implemented; it
returns ranked `path:line` hits with snippets in ~2-5s.

**Default to perseus, not grep, for locating.** If your next step is "find the
file/function that does X," that's a perseus query. Grep is for occurrences of
a string you already know — not for locating behaviour.
...
It auto-indexes the current branch/commit on first use — just run it from
inside the repo. Re-run `perseus index` after large changes if hits look stale.
<!-- /perseus:rules -->
